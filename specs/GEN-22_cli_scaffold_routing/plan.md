...
| II. Testing Standards | PASS | Plan requires integration tests (`assert_cmd`) written before stub handlers, one per acceptance scenario, per TDD. `.github/workflows/ci.yml` now runs `cargo test --all` on 3 native GitHub-hosted runners (`ubuntu-latest`, `macos-latest`, `windows-latest`), covering Linux/macOS/Windows exactly as the Quality Gates' "run on all supported target platforms (Linux, macOS, Windows)" clause names, plus a `cargo check --target aarch64-unknown-linux-gnu` compile-only job for the 4th target triple (no free-tier native ARM Linux GitHub runner exists). This closes the deviation previously documented below for this same principle; a full *execution* matrix across all 4 target triples (including cross/QEMU emulation for `aarch64-unknown-linux-gnu`) remains GEN-31's dedicated CI-matrix scope |
...
**Previously-documented deviation, now resolved**: an earlier draft of this plan noted that full `cargo test --all` execution was gated on the native CI platform only (manual runs on macOS aarch64), with the remaining three target triples covered by compile-only `cargo check --target <triple>` checks (T051), because no CI matrix existed yet — and flagged that this required explicit maintainer sign-off before merge, not yet obtained as of that draft. `.github/workflows/ci.yml` (added during post-review remediation) closes this gap: `cargo test --all` now runs automatically on every push/PR across `ubuntu-latest`/`macos-latest`/`windows-latest` (the 3 platforms the Quality Gates name by name), with `aarch64-unknown-linux-gnu` covered by an automated compile-only check in the same workflow (previously only ever run manually, per T051). No remaining Constitution Check violation exists for this ticket; the Complexity Tracking entry below is retained as a historical record of the now-closed gap. A full execution matrix across all 4 target triples (real/emulated hardware for `aarch64-unknown-linux-gnu`) remains GEN-31's dedicated scope.
...
>
> **Historical record — this deviation is now closed.** It applied only
> until `.github/workflows/ci.yml` was added during post-review
> remediation; see the Constitution Check table's Principle II row above
> for the current, resolved state. Retained here rather than deleted so
> the rationale that justified merging before a full CI matrix existed
> stays auditable.

| Violation (as it stood before `.github/workflows/ci.yml` existed) | Why It Was Needed | Simpler Alternative Rejected Because |
|-----------|------------|---------------------------------------|
| Quality Gates' "full test suite ... run on all supported target platforms (Linux, macOS, Windows)" was satisfied on only the native CI platform (manual runs, T052), not all four target triples | No CI matrix for the four target triples (`x86_64-pc-windows-msvc`, `aarch64-apple-darwin`, `aarch64-unknown-linux-gnu`, `x86_64-unknown-linux-gnu`) existed in this repo yet; standing one up was originally deferred to GEN-31's dedicated, explicit scope. Gating this scaffolding PR on infrastructure that didn't exist yet would have blocked an otherwise-correct, verified change indefinitely | Skipping compile verification entirely on the non-native triples (leaving only native `cargo test`) would have silently regressed platform coverage with no signal at all; skipping the native-platform full-suite run entirely (the ticket's original scope) would have left zero automated test-execution gate before merge, which the Quality Gates do not permit as a silent gap. Combining native `cargo test` (T052) + all-triple `cargo check` (T051) was the closest approximation available without GEN-31's infrastructure — superseded once the minimal CI matrix below was added |
