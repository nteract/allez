...
"Resolved Channel Configuration." `crates/condarc` has no dependency on
`allez` (research.md R7) — this type's own design never assumed one —
but `allez`'s own environment-creation capability (GEN-24) now consumes
it directly as its own channel-configuration input (FR-012/FR-016,
research.md R15): `allez::ephemeral`'s previously-separate
`ChannelConfig`/`ChannelSpec`/`ChannelPriorityMode` types are retired
rather than kept as a second, `allez`-shaped duplicate. This closes out
both the PR #5 review thread requesting struct reuse and the PR #6
thread (`discussion_r3694858120`) floating this type's eventual
convergence with `allez`'s own types — the crate-side resolved type is
now the only resolved-channel-configuration type in the workspace.
...
/// This is the sole implementation of this policy in the workspace —
/// `allez`'s own former copy, `filter_channels()`, is retired (FR-016,
/// research.md R15) now that this crate-local pass already produces the
/// final, filtered list.
...
## `allez` — file handling and channel-configuration retirement

New module tree, `src/channel_config/` (sibling to the existing
`src/ephemeral/`, `src/cli/`, `src/error.rs`, `src/observability.rs`,
`src/output.rs`). `src/ephemeral/channels.rs` is directly modified as
part of this same ticket (FR-016, research.md R15): `ChannelConfig`,
`ChannelSpec`, `ChannelPriorityMode`, `filter_channels`, and
`channels_with_fallback` are removed; `redact_channel_url` survives,
applied explicitly at each channel-identifier format site.

### `resolve_channel_config` / `resolve_channel_config_from`

```rust
/// Locates and reads `~/.condarc` (via [`dirs::home_dir`]), parses and
/// resolves it through the `condarc` crate, and hands the result
/// directly to GEN-24's ephemeral-environment-creation capability as
/// its own channel-configuration input (FR-009–FR-011, FR-012), paired
/// with an explicit fallback signal (FR-017), or reports FR-020's
/// zero-usable-channels case instead of ever constructing an
/// intentionally-empty channel configuration.
///
...
/// R13) — never handed to GEN-24 as an empty channel configuration.
/// Resolves fresh from disk on every call — never caches or reuses a
/// previous result (FR-015).
...
/// `Ready { config: resolved, fallback }`.
...
ordinary case, `Ready`, pairs the crate's own `ResolvedChannels`
...
pub enum ChannelConfigResolution {
    ...
    /// no channel configuration for a caller to mistakenly pass to
    /// GEN-24.
    NoChannels,
}
```
...
`allez::channel_config::FallbackReason`, so an external caller (GEN-25)
...
ChannelConfigResolution::Ready   1---1 condarc::ResolvedChannels (the `config` field, GEN-24's own required channel-configuration input directly, research.md R15)
