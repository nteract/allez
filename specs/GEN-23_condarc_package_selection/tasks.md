...
- [ ] T003 [P] Create the `src/channel_config/` module tree — `mod.rs`, `locate.rs`, `events.rs`, each with a module-level doc comment and no public items yet — and add `pub mod channel_config;` to `src/lib.rs` (plan.md Project Structure)
...
- [ ] T077 [P] Add `ResolvedChannels::from_channels(channels: Vec<String>) -> Self` (strict priority) in `crates/condarc/src/expand_channels.rs`, mirroring the retired `ChannelConfig::from_urls`'s own behavior exactly (data-model.md, research.md R15) — needed so external callers (`allez`'s own tests, `examples/ephemeral_smoke.rs`) can construct a `ResolvedChannels` directly despite `#[non_exhaustive]`
- [ ] T078 Remove `ChannelConfig`, `ChannelSpec`, `ChannelPriorityMode`, `filter_channels`, and `channels_with_fallback` from `src/ephemeral/channels.rs`; simplify `resolve_channel_source` to a passthrough (every identifier `expand_channels()` produces is already a fully-qualified URL, so the literal `"defaults"` name never reaches this function any more) and remove the now-dead `DEFAULTS_CHANNEL_NAME`/`DEFAULTS_CHANNEL_URL` constants along with it (both are used only by the functions this task removes); add `redact_channel_url` to `src/ephemeral/mod.rs`'s existing `pub use channels::{...};` re-export list, removing the three retired types from that same list — depends on T065, T077
- [ ] T079 [P] Rewrite `channel_config_from_urls_defaults_policy` in `channels.rs`'s own test module to assert against `condarc::ResolvedChannels::from_channels` instead — depends on T078
- [ ] T080 [P] Remove `channel_spec_debug_redacts_credentials` and `channel_config_debug_redacts_credentials_in_all_channel_fields` from `channels.rs`'s own test module (the types they test no longer exist); `redact_channel_url_removes_userinfo_and_conda_tokens_and_preserves_clean_values` is unaffected and stays — depends on T078
- [ ] T081 [P] Remove `channel_fallback_substitutes_defaults_only_when_empty` from `channels.rs`'s own test module (`channels_with_fallback` no longer exists); its coverage is superseded by `NoChannels` (FR-020, already tested by T049/T062) — depends on T078
- [ ] T082 [P] Remove `filter_channels_denies_before_applying_allowlist` and `filter_channels_without_allowlist_only_applies_denials` from `channels.rs`'s own test module (`filter_channels` no longer exists); their coverage is superseded by `apply_allow_deny`'s own crate-level tests (T076) — depends on T078
- [ ] T083 Update `src/ephemeral/mod.rs`'s `create_ephemeral_environment(requested, channels: condarc::ResolvedChannels, default_override)` signature and `src/ephemeral/solve.rs`'s `solve_packages(root, config: &condarc::ResolvedChannels, packages)` to take `condarc::ResolvedChannels` directly; `solve_packages` uses `config.channels` as-is (already deny-then-allow filtered, FR-019) instead of calling `channels_with_fallback`/`filter_channels`, erroring `EphemeralEnvError::NoChannelsConfigured` only when it receives an empty `channels` list directly; also migrate `solve.rs`'s private `solver_priority(priority: ChannelPriorityMode) -> rattler_solve::ChannelPriority` helper to take `condarc::ChannelPriority` instead, preserving the existing `Strict -> Strict`/`{Flexible, Disabled} -> Disabled` mapping with a wildcard arm covering any future `#[non_exhaustive]` variant the same way `Flexible` is covered today — depends on T078
- [ ] T084 [P] Update `solve.rs`'s own test module and `install.rs`'s/`examples/ephemeral_smoke.rs`'s test/example fixtures: replace every `ChannelConfig`/`ChannelPriorityMode`/`ChannelConfig::from_urls` reference with `condarc::ResolvedChannels`/`condarc::ChannelPriority`/`ResolvedChannels::from_channels` — depends on T083, T077
- [ ] T086 [P] Update every `tests/support/*.rs` fixture that constructs or matches on `ChannelConfig`/`ChannelSpec`/`ChannelPriorityMode` (`ephemeral.rs`, `user_story_1_creation.rs`, `user_story_1_failures.rs`, `user_story_2_reap.rs`, `user_story_3_defaults.rs`) to build `condarc::ResolvedChannels` (via `ResolvedChannels::from_channels` or a struct literal) instead — these back the `tests/ephemeral_env.rs`/`tests/cli_scaffold.rs` integration suites, which do not compile once T078/T083 land without this migration — depends on T083, T077
- [ ] T087 Run `cargo test --lib ephemeral::` and `cargo test --all` and confirm zero regressions beyond the deliberate rewrites above — depends on T077, T078, T079, T080, T081, T082, T083, T084, T086
