...
def main() -> int:
    ...
    # `key_exceeds_yaml_simple_key_length_limit` above (in `REJECT`) documents that real conda
    # rejects a 1023-character raw key outright, one character past `ruamel.yaml`'s
    ...
    # mirror-image `accept_key_exceeds_yaml_simple_key_length_limit` fixture (identical key/value
    # shape) exists to pin the crate's own permissive behavior on its own terms.
    #
    # It cannot go through `generate_battery` like every other candidate above: that helper only
    # writes a candidate when conda's own verdict matches the battery's `expect_valid`, and conda
    # genuinely rejects this one (that's the entire point) -- so folding it into `ACCEPT` would
    # make `generate_battery` itself `SKIPPED`/never write it, silently reproducing the exact bug
    # this unconditional write exists to fix (`clear_stale` deletes the previously-written file,
    # nothing recreates it). Named in `tests/condarc_conformance.rs`'s `RUST_ONLY_FIXTURES` (the
    # `Conda`/`OpenApi` checkers are skipped entirely for it, before running anything -- there is
    # no real conda verdict to self-verify against here). Its `expected/*.json` sibling is
    # likewise hand-authored, by `generate_zzz_condarc_expected_fixtures.py`'s
    # `CRATE_ONLY_YAML_KEY_LENGTH_FIXTURES`, not generated from a live conda read.
    crate_only_name = (
        "custom_multichannels_values_accept_key_exceeds_yaml_simple_key_length_limit.json"
    )
    crate_only_doc = {KEY: {"k" * 1023: ["a"]}}
