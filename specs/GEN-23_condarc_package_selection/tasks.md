...
- [ ] T004 Define the `ResolvedChannels` struct (`#[non_exhaustive]`, `channels: Vec<String>`, `channel_priority: ChannelPriority`, `Clone, PartialEq, Eq` — no `Debug` derive, see T075) in `crates/condarc/src/expand_channels.rs` (data-model.md)
...
- [ ] T010 Define the `ChannelConfigResolution` enum (`#[non_exhaustive]`, `Debug, Clone, PartialEq`, `Ready { config: ChannelConfig, fallback: Option<FallbackReason> }`, `NoChannels`) in `src/channel_config/mod.rs`, plus `pub use events::FallbackReason;` (data-model.md, research.md R13) — depends on T009
...
- [ ] T012 [P] Implement `adapt(resolved: condarc::ResolvedChannels) -> ChannelConfig` in `src/channel_config/adapt.rs`: direct field-by-field mapping (`channels` -> `Vec<ChannelSpec>`, `channel_priority` matched `Strict`/`Disabled` explicitly, every other value including `Flexible` via a wildcard arm to `ChannelPriorityMode::Flexible`, `allowed_channels`/`denied_channels` always `Vec::new()`) — FR-012, data-model.md — depends on T001 and T008 (needs `condarc` as a runtime dependency and `ResolvedChannels` re-exported before this file can name it) and the existing `crate::ephemeral::{ChannelConfig, ChannelSpec, ChannelPriorityMode}`; no dedicated test in this phase (see Phase 2 Purpose above) — covered by T048
- [ ] T074 Unit test for `ResolvedChannels`'s `Debug` impl in `crates/condarc/src/expand_channels.rs`: a `ResolvedChannels` with `channels: vec!["https://user:pass@example.com/x".into(), "https://example.com/t/abc123/y".into()]` asserts the `{:?}`-formatted output equals `ResolvedChannels { channels: ["https://example.com/x", "https://example.com/y"], channel_priority: Flexible }` — both entries redacted, neither `user:pass@` nor `/t/abc123/` present — depends on T004; same file as T005-T008, so not `[P]`
- [ ] T075 Implement `redact_channel_credentials(raw: &str) -> String` (this crate's own private redaction primitive: for every substring matching the scheme pattern `[a-z][a-z0-9]{0,11}://` up to the next whitespace/quote, strip any `user[:pass]@` userinfo and any `/t/<segment>/` path component) and a manual `impl std::fmt::Debug for ResolvedChannels` calling it over each `channels` entry, in `crates/condarc/src/expand_channels.rs` (data-model.md, research.md R14) — independent of the identically-named, identically-behaved private function in `src/channel_config/events.rs` (T055); this crate cannot depend on `allez` (research.md R7) — depends on T004, T074; makes T074 pass; same file as T004-T008, so not `[P]`

**Checkpoint**: `cargo build --all` succeeds; both new public surfaces (`condarc::expand_channels`/`ResolvedChannels`/`ExpandChannelsError`, `allez::channel_config`'s types) exist and compile, with `expand_channels()` still a stub, `adapt()` fully implemented, and `ResolvedChannels`'s redacting `Debug` impl (T075) in place.
...
> Every test below targets a function that does not exist yet in this phase, except T048, which is `adapt()`'s deliberate after-the-fact test (Phase 2 Purpose). Confirm each other one fails (or fails to compile) before starting the Implementation subsection.
...
- [ ] T040 [P] [US2] FR-009 test (grouped with SC-004's observability tests since it's the negative case of the same assertion): `resolve_channel_config_from` driven against a missing file emits **zero** `ChannelConfigFallbackEvent`, in `src/channel_config/mod.rs`
- [ ] T041 [P] [US2] SC-007 test (end-to-end half): `resolve_channel_config_from` driven against a rejected fixture whose content embeds a URL userinfo segment or access-token path segment asserts the captured `ChannelConfigFallbackEvent.detail` is already redacted, in `src/channel_config/mod.rs` — the unreadable path's `detail` comes from `io::Error`'s own `Display` text, never from `.condarc` content, so it cannot exercise redaction and is intentionally excluded from this fixture choice
...
- [ ] T048 [US2] SC-001 contract test: 5 distinct, real-world-shaped `.condarc` samples, each parsed via `condarc::parse`, resolved via the real `condarc::expand_channels`, adapted via `adapt()`, asserted equal to a hand-typed `ChannelConfig` literal (`allowed_channels`/`denied_channels` always `Vec::new()`, `channel_priority: ChannelPriorityMode::Flexible` unless stated) — run on all four target platforms per plan.md's Technical Context, so each sample below states both its non-Windows and Windows expected list — none of these five samples sets `allowlist_channels`/`denylist_channels` (that sample lands in Phase 5's T063 instead, once allow/deny filtering exists — see T063's own note):
  1. `channels: [conda-forge, defaults]` → non-Windows `["https://conda.anaconda.org/conda-forge", "https://repo.anaconda.com/pkgs/main", "https://repo.anaconda.com/pkgs/r"]`; Windows `["https://conda.anaconda.org/conda-forge", "https://repo.anaconda.com/pkgs/main", "https://repo.anaconda.com/pkgs/r", "https://repo.anaconda.com/pkgs/msys2"]`.
  2. `channels: [acme/label/dev, team]`, `custom_channels: {acme: "https://internal.example.com"}`, `custom_multichannels: {team: ["https://other.example.com/x", member2]}` → `["https://internal.example.com/acme/label/dev", "https://other.example.com/x", "https://conda.anaconda.org/member2"]` on every platform (no `defaults` substitution involved, so no Windows variance).
  3. `channels: [defaults, https://custom.example.com/chan]` → non-Windows `["https://repo.anaconda.com/pkgs/main", "https://repo.anaconda.com/pkgs/r", "https://custom.example.com/chan"]`; Windows `["https://repo.anaconda.com/pkgs/main", "https://repo.anaconda.com/pkgs/r", "https://repo.anaconda.com/pkgs/msys2", "https://custom.example.com/chan"]` (the `msys2` entry inserts where `defaults` expanded, before the next `channels` entry — never appended after it).
  4. `channels: [conda-forge]`, `channel_priority: true` → `["https://conda.anaconda.org/conda-forge"]` on every platform, `channel_priority: ChannelPriorityMode::Flexible` (legacy boolean `true` → `flexible`, FR-003).
  5. `{}` (empty document, `Config::default()`) → non-Windows `["https://repo.anaconda.com/pkgs/main", "https://repo.anaconda.com/pkgs/r"]`; Windows `["https://repo.anaconda.com/pkgs/main", "https://repo.anaconda.com/pkgs/r", "https://repo.anaconda.com/pkgs/msys2"]`; `channel_priority: ChannelPriorityMode::Flexible`.

  In `src/channel_config/adapt.rs` — exercises T012's `adapt()` and Phase 3's `expand_channels()` (T033); this is `adapt()`'s only test in this phase (Phase 2 Purpose)
...
- [ ] T055 [US2] Implement `redact_channel_credentials(raw: &str) -> String` and `redact_and_bound(raw: &str) -> String` in `src/channel_config/events.rs`: `redact_channel_credentials` does the redaction itself — for every substring matching the scheme pattern `[a-z][a-z0-9]{0,11}://` up to the next whitespace/quote, strip any `user[:pass]@` userinfo and any `/t/<segment>/` path component, applied independently to every matched substring; `redact_and_bound` calls it, then truncates the result to `MAX_FALLBACK_DETAIL_LEN` (2048) at the nearest UTF-8 character boundary (FR-021, research.md R9) — this module's own private implementation, independent of the identically-named, identically-behaved private function in `crates/condarc/src/expand_channels.rs` (T075) — makes T035 pass
- [ ] T056 [US2] Define `const CHANNEL_CONFIG_EVENT_SCHEMA_VERSION: &str = "1"` and implement `ChannelConfigFallbackEvent` (`schema_version: &'static str`, `reason: FallbackReason`, `detail: String`, constructed with `schema_version: CHANNEL_CONFIG_EVENT_SCHEMA_VERSION`) and `emit_fallback(reason: FallbackReason, detail: &str)` in `src/channel_config/events.rs`, passing `detail` through `redact_and_bound` and emitting via `tracing::warn!` through the existing `src/observability.rs` pipeline (FR-011, research.md R9) — depends on T055; makes T036 pass
- [ ] T057 [US2] Implement `resolve_channel_config_from(path: Option<&Path>) -> ChannelConfigResolution` in `src/channel_config/mod.rs`: `path` of `None` or a `read_condarc` `Missing` result falls back silently (`fallback: None`, no event); a `read_condarc` `Unreadable` result, a `condarc::parse` rejection, or a `condarc::expand_channels` `Err` all fall back to `condarc::expand_channels(&Config::default())`, unwrapped via `.expect("expand_channels(&Config::default()) is documented to never fail; if it does, the crate's own built-in defaults changed incompatibly")` — this call is guaranteed to succeed per FR-002's defaulting table (data-model.md, research.md R15) and a deliberate panic on that invariant, not a silent `NoChannels`/fabricated reason, is the specified behavior if it ever didn't — then call `emit_fallback` with the matching `FallbackReason` (`Unreadable` vs `Rejected`, the latter also covering an `expand_channels` `Err` per research.md R11) and the per-problem detail text; on any successful `expand_channels()` result (real or fallback), an empty `resolved.channels` maps to `ChannelConfigResolution::NoChannels`, otherwise `Ready { config: adapt(resolved), fallback }` (FR-009/FR-010/FR-011/FR-012/FR-017/FR-018/FR-020) — depends on T053, T054, T056, T012; makes T037–T047, T049–T052 pass
...
- [ ] T059 [P] [US3] SC-003 scenarios 11–16 (`channel_priority` = `strict`/`flexible`/`disabled`, absent-defaults-to-`flexible`, legacy boolean `true`/`false`) tests in `crates/condarc/tests/expand_channels_scenarios.rs` — depends on T016 (FR-003); already-correct passthrough from Phase 3/T033, so these pass immediately
...
- [ ] T062 [US3] SC-006 test: a `.condarc` whose `channels` is non-empty before filtering but whose `allowlist_channels`/`denylist_channels` remove every entry asserts `resolve_channel_config_from` returns `ChannelConfigResolution::NoChannels` — not `Ready` with an empty `ChannelConfig` — in `src/channel_config/mod.rs` (FR-020, research.md R13) — targets behavior delivered by T057, T065; fails until T065 lands
- [ ] T063 [US3] Add a sixth SC-001 sample to the `adapt()` contract test in `src/channel_config/adapt.rs`: `channels: [alpha, beta, gamma]`, `channel_alias: https://example.com`, `allowlist_channels: [alpha, beta]`, `denylist_channels: [beta]` — deny-then-allow leaves only `alpha` — asserted equal to `ChannelConfig { channels: [ChannelSpec { url_or_name: "https://example.com/alpha" }], channel_priority: ChannelPriorityMode::Flexible, allowed_channels: Vec::new(), denied_channels: Vec::new() }` — targets behavior delivered by T048, T065; fails until T065 lands
- [ ] T076 [US3] Unit tests for `apply_allow_deny()` in `crates/condarc/src/expand_channels.rs`: empty-allowlist passthrough (deny-only filtering), deny-and-allow together, a channel present in both lists removed (deny wins), and survivor order plus a coincidental duplicate (FR-007) both preserved — same file as T064/T065, so not `[P]`; fails until T064 lands
...
- Within Foundational: T009 and T011 run in parallel with each other and with the T004→T008 chain; T012 (`adapt()`) can start once T001 and T008 land (not T004 alone — see T012's own dependency note); T074 then T075 run sequentially after T004 lands, in the same file as T005–T008 so neither is `[P]`.
